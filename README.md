...
- **Play.** Sounds one plotted seed, chosen in the Hearing menu. The browser synthesizes a piano-like tone (no samples and no server). Right hand plays odd-exponent prime powers, slightly louder and brighter. Left hand plays the other terms half a step later and softer. When the path reaches a pure power of 2, both hands land on the same pitches and descend through 4 → 2 → 1, then the last note rings. Pitch is log₂ of the value, compressed into C2–C6 and snapped to a C-major pentatonic (C D E G A) so the contour stays consonant. ms/step sets the tempo. This sonification is exploratory. It is not a proof.
