...
- **Play.** Sounds one plotted seed, chosen in the Hearing menu. The browser synthesizes a piano-like tone (no samples and no server). The right hand states each odd-exponent prime power on the beat, a clear motif. The left hand rolls the other terms afterward: a low note, a fifth above it, then that term’s pitch, staggered through the step. When the path reaches a pure power of 2, the roll stops and both hands land together, descending through 4 → 2 → 1 until the last note settles. Pitch is log₂ of the value, compressed into C2–C6 and snapped to a C-major pentatonic (C D E G A). Those figures are original. They are not a quotation of an existing piece. ms/step sets the tempo. This sonification is exploratory. It is not a proof.
